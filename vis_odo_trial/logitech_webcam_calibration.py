...
while True:
    ...
    ret_val_left, corners_left = cv2.findChessboardCorners(gray_image_left, (6, 8), None)
    ret_val_right, corners_right = cv2.findChessboardCorners(gray_image_right, (6, 8), None)
...
# Do the calibration.
retval, K_left, distCoeffs_left, K_right, distCoeffs_right, R, T, E, F = cv2.stereoCalibrateCamera(
    objectPoints=objpoints, imagePointsLeft=imgpointsLeft, imagePointsRight=imgpointsRight, imageSize=(w, h),
    cameraMatrix=None, distCoeffs=None)
print("ret_val left: ", ret_val_left)
print("ret_val right: ", ret_val_right)
...
for i in range(len(objpoints)):
    imgpoints_left, _ = cv2.projectPoints(objpoints[i], rvecs[i], T[i], K_left, distCoeffs_left)
    imgpoints_right, _ = cv2.projectPoints(objpoints[i], rvecs[i], T[i], K_right, distCoeffs_right)
