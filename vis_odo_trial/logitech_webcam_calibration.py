...
images = glob.glob('../data/original/*.png')     # Get list of filenames in this folder
for fname in images:
    img = cv2.imread(fname)
    gray_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    h, w = gray_image.shape
    # Find the chess board corners
    ret_val, corners = cv2.findChessboardCorners(gray_image, (6, 8), None)
...
criteria)
        ...
        cv2.drawChessboardCorners(img, (6,8), corners2, ret_val)
        cv2.imshow('img', img)
        cv2.waitKey(0)
